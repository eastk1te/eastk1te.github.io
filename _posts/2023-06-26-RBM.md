...
Restricted Boltzmann Machine$_{RBM}$은 심층 확률 모형을 구축하는 데 가장 흔이 사용되는 요소이며, ANN, DNN, CNN, RNN 등과 같은 타겟과 가설 간의 차이를 줄여서 오차를 줄이는 것이 목표인 Deterministic Model들과는 다르게 확률 밀도 함수를 모델링 하는 것이 목표이다.


- CNN : 영상 인식, 컴퓨터비전
- RNN : 음성인식, 작사 작곡, 텍스트 처리, 주가예측
...
1. [temp](https://www.cs.toronto.edu/~amnih/papers/rbmcf.pdf)
1. temp
1. temp
1. temp
1. temp
